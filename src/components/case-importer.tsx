...
import { buildRows, guessField, type ParsedRow } from "@/lib/case-import";
import { importConfig } from "@/lib/case-import-registry";

const IGNORE = "__ignore__";

/**
 * Importador de planilha genérico: recebe o slug do agravo e usa a
 * configuração de campos/tabela correspondente.
 */
export function CaseImporter({ agravo, onImported }: { agravo: string; onImported?: () => void }) {
  const config = importConfig(agravo);
  const { user } = useAuth();
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (fichas.length > 0) {
        ...
        existing = new Set(
          ((data ?? []) as { numero_ficha: string | null }[])
            .map((d) => d.numero_ficha)
      ...
      const payloads = toInsert.map((r) => ({ ...r.payload, ...(config.extra ?? {}), user_id: user.id }));
      const CHUNK = 200;
      let inserted = 0;
  ...
  return (
    ...
    >
      ...
      <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Importar planilha — {config.title}</DialogTitle>
          <DialogDescription>
          ...
          <div className="space-y-3">
            ...
            {missingRequired.length > 0 && (
              <div className="flex items-start gap-2 text-sm text-destructive border border-destructive/30 bg-destructive/5 rounded-lg p-3">
                ...
                <span>Campos obrigatórios não mapeados: {missingRequired.map((f) => f.label).join(", ")}</span>
              </div>
            )}
